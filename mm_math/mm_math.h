...
public:
    ...
    */ 
    static float get_y_rotation(const Quaternion &quat);
    ...
    // Quaternion must be normalized before
    static Vector3 quat_to_angular_velocity(const Quaternion &quat);

    static Quaternion quat_from_angular_velocity(const Vector3 &vec);

    static void simple_spring_damper_implicit_quat(Quaternion &x, Vector3 &v, const Quaternion &goal, float halflife, float delta);

    // Angular velocity is represented as rotation axis scaled by rotation angle
    // With eydt as a parameter for fast calculation
    static void inertialize_update_quat(Quaternion &out_quat, Vector3 &out_velocity, Quaternion &off_quat, Vector3 &off_velocity, const Quaternion &in_quat, const Vector3 &in_velocity, float halflife, float delta, float eydt);
    ...
    // With eydt as a parameter for fast calculation
    static void decay_spring_damper_implicit_quat(Quaternion &quat, Vector3 &velocity, float halflife, float delta, float eydt);
    ...
    static Quaternion minial_angle_quat(const Quaternion &quat);
